...
- Name: security\rbac-matrix.spec.ts >> RBAC Security Matrix @security >> Role: MANAGER >> CAN access /api/leaves/1/approve
- Location: tests\security\rbac-matrix.spec.ts:28:13

# Error details

```
Error: [ApiClient] Auth file not found for role "manager". Did global setup run?
```
